    ...
    return (
        <div className={`${jarkata.className}`}>
            <Link href={`editorial/${slug}`} prefetch className='flex flex-col gap-4'>
                ...
                <div className='flex-1'>
                    <p className='text-sm sm:text-base text-black-3'><span className='text-[#9B9B9B] mr-1'>BY </span><span>{by}</span></p>
                    <h3 className='text-xl sm:text-[2rem] leading-8 sm:leading-[3rem] line-clamp-2 font-bold text-black-3'>{title}</h3>
