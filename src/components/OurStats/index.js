        ...
        <div className={`border border-1 border-[#90CCD4] w-full rounded rounded-[30px] overflow-hidden ${jarkata.className} mt-6`}>
            <div className='w-full flex flex-col sm:flex-row flex-wrap'>
                {stats.map((item, index) => (
                    <div className={`flex-1  ${index === stats.length - 1 ? "" : 'border border-r-1 border-[#90CCD4]'}  flex justify-between px-6 py-5 sm:py-8`} key={index}>
            ...
            <div className='w-full flex-col sm:flex-row flex items-center sm:border sm:border-t-1 sm:border-t-[#90CCD4]'>
